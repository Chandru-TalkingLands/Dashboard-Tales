...
// >>>>>>> 537319db6d11fd7500d44e35dea5ae8608128a9c
  ...
  // console.log(num)
  /*
 // With Delete Image from AWS S3 Bucket Working Code from next Line
  const fileInputRef = useRef(null);
  const [value, setValue] = useState("Amenities");
  const [pushdata, setpushdata] = useState([]);
  const [data, setData] = useState({
  ...
  const handleDelete = (e, position) => {
    ...
    axios.delete(`http://localhost:7000/delete/file/${property_name}/${sub_folder}/${category_name}/${file_key}`)
